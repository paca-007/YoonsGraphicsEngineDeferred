...
namespace ASEParser
{
	...
	struct Vertex
	{
		...
		//버텍스를 복사해주는 친구. 
		// normal값이나 texture값 때문에 새로운 vertex가 필요할때 쓰인다.
		Vertex* CopyVertex() const
		{
			Vertex* newVertex = new Vertex;
			newVertex->m_pos = this->m_pos;
			newVertex->m_normal = this->m_normal;
			newVertex->m_isnormalset = this->m_isnormalset;
			newVertex->m_bw1 = this->m_bw1;
			newVertex->m_bw2 = this->m_bw2;
			newVertex->m_bw3 = this->m_bw3;


			newVertex->m_indices = this->m_indices;
			newVertex->m_indicesPointer = this->m_indicesPointer;
	...
	struct Face
	{
		...
		int				m_normalvertexindex;
		//normalVertex의 index값.
		// 초기화를 위한 생성자.
		Face();
	};
	...
	public:
		...
		vector<int> m_tempTFace0;		//tFace저장해둘 임시 vector
		vector<int> m_tempTFace1;		//tFace저장해둘 임시 vector
		vector<int> m_tempTFace2;		//tFace저장해둘 임시 vector
